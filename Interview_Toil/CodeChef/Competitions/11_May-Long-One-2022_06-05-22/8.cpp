// https://www.codechef.com/MAY221D/problems/MAGICSTONE
#include <bits/stdc++.h>
...
auto dp_fact = new ll[MAX_N];
auto dp_inv = new ll[MAX_N];

ll select(ll l, ll r) {
    ll res;
    if (r > l || l < 0 || r < 0) return 0;
    res = (dp_fact[l] * dp_inv[r]) % MOD;
    res = (res * dp_inv[l - r]) % MOD;
    return res;
}

ll pascal_pow(ll a, ll b) {
      ll res = 1;
      if (b < 0) return 0;
      while (b) {
...
int32_t main() {
   FAST();
   // pre computation
   dp_fact[0] = 1, dp_inv[0] = 1;
   for (ll i = 1; i < MAX_N; i++) {
      dp_fact[i] = (dp_fact[i - 1] * i) % MOD;
      dp_inv[i] = pascal_pow(dp_fact[i], MOD - 2);
   }
   testcases { 
      ll N, L, R;
      cin >> N >> L >> R;
      for(ll i = L; i <= R; ++i) {
         ...
         ll check = (i+N)%2;
         ll pass = (i+N)/2;
         if(check==0) { res = select(N, pass); }    // control flow change
         cout << res << " ";
      }
      cout << endl;
   }
