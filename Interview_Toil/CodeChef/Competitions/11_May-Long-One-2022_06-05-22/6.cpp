// https://www.codechef.com/MAY221D/problems/XORGAND
...
//     		ll cnt{0};
//     		for (ll i = L-1; i < R; ++i) {
//                 ull xor_part = (vect[i]^X)%MOD;
//                 ull and_part = (vect[i]&X)%MOD;
//     			if(xor_part > and_part) {
//     				cnt++;
//     			}
//     		}
//     		cout << cnt << endl;
...
int32_t main()
{
    ...
    testcases {
        ...
        for(int i=1;i<=N;i++){
            ...
            int powers = 32;
            if(var!=0) { powers = (int)(log(var)/log(2)); }
            my_dp[i][powers]++;
        }
        ...
        testcases {
            ...
            if(X!=0) { HELL = (int)(log(X)/log(2)); }
            
            cnt = my_dp[R][HELL]-my_dp[L-1][HELL];
