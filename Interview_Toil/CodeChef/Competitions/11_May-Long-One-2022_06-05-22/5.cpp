// https://www.codechef.com/MAY221D/problems/PUSH7PA
...
int32_t main()
{
    ...
    // 	}
    // 	utility_sort(vect);
    // 	int max_at_last{vect.back()}, cnt{0};
    // 	for(int i = 0; i < vect.size() - 1; i++) {
    // 		if(vect[i] == max_at_last){ cnt++; }
    // 	}
    // 	cout << max_at_last + cnt << endl;
    // }
    ...
    // 	cin >> N;
    // 	vi vect(N);
    // 	auto max_at_last = INT_MIN;
    // 	for(auto &x: vect) {
    // 		cin >> x;
    // 		max_at_last = max(x, max_at_last);
    // 	}
    // 	utility_sort(vect);
    // 	ll cnt{0};
    // 	for(ll i = 0; i < N; i++) {
    ...
    testcases {
    	...
    	hashmap<ll, ll> my_map;
    	for (int i = 0; i < N; ++i) {
    		my_map[vect[i]]++;
    	}

    	int sz = my_map.size();
      vector<ll> my_list(sz);
    	int j{0};

    	for(auto ele: my_map) {
         my_list[j] = (ele.first + ele.second - 1);
    		j++;
    	}
      utility_sort(my_list);
    	cout << my_list[sz-1] << endl;
    }
    return 0;
}
