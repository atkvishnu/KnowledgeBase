...
int32_t main()
{
    ...
    testcases {
      ...
      if (B < W) { swap(B,W); swap(my_vect[0], my_vect[1]); }
      if (W == 0 && B > 1) { cout << "-1" << endl; continue; }

      int var = B + W;
      
      if (var >= 1) { cout << my_vect[0]; }
      if (var >= 2) { cout << my_vect[1]; }
      if (var >= 3) { cout << my_vect[0]; }

      W--; B -= 2;

      for (int i = 0; i < W; i++) { cout << my_vect[1]; }
      for (int i = 0; i < B; i++) { cout << my_vect[0]; }
