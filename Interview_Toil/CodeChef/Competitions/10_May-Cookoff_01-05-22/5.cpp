...
int32_t main()
{
    ...
    testcases {
    	...
    	sort(vect.begin(), vect.end(), comp_desc);
    	// cout << vect <<endl;
    	int cnt = 0;
    	for(int i = 0; i < N - 1; ++i){
    		int curr_minus_prev = vect[i] - vect[i-1];
    		int two_forward_minus_current = 2 * (vect[i+1] - vect[i]);
    		int current_minus_prev = 2 * (vect[i] - vect[i-1]);
    		int forward_minus_current = vect[i+1] - vect[i];
    		cnt += (curr_minus_prev == two_forward_minus_current) || (current_minus_prev == forward_minus_current);
    	}
    	if(cnt == N-2) {
    			cout << "YES\n";
		} else {
