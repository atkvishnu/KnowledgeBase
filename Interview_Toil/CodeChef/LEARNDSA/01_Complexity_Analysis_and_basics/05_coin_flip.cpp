...
// i -> initial state (1-tails/2-heads)
// n -> number of coins and rounds
// q -> final state (1-tails/2-heads)
void solve(int i, int n, int q){		// TLE
	int arrCoins[n];		// declaring array of size n
	int szArr = sizeof(arrCoins)/sizeof(arrCoins[0]);	// = n (i.e., 5)
	
    for(int j = 0; j < szArr; j++) {
    	arrCoins[j] = i;		// initially making all the coins as i
    	// cout << arrCoins[j] << " ";
    }
    // cout << endl;
