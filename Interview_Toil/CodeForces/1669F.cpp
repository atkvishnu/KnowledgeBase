...
// ll utility_binary_to_decimal(string s) { ll res = stoi(s, 0, 2); return res; }
...
// inline ll utility_getFactorial(int n){ const unsigned int M = 1000000007; unsigned ll f = 1;for (int i = 1; i <= n; i++){ f = (f*i) % M; } return f; }
...
int32_t main()
{
    ...
    testcases {
    	...
    	prefix_sum[0] = vect[0];
    	suffix_sum[n-1] = vect[n-1];

    	for (int i = 1; i < n; i++){
            prefix_sum[i] = prefix_sum[i - 1] + vect[i];
    	}
        for (int i = n - 2; i >= 0; i--){
            suffix_sum[i] = suffix_sum[i + 1] + vect[i];
        }

        reverse(all(suffix_sum));

        int ans = 0;
        for (int i = 0; i < n; i++) {
            int weight_alice = prefix_sum[i];
            int cnt_alice = (i + 1);

            if (binary_search(all(suffix_sum), weight_alice))
            {
                int cnt_bob = lower_bound(all(suffix_sum), weight_alice) - suffix_sum.begin() + 1;
                if (cnt_alice + cnt_bob <= n)
