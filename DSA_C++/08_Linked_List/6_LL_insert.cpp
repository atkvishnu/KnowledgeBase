...
//===================DISPLAY=======================================
void Display(struct Node *p)                            // struct Node *p -> is the pointer to the first node!
{
    while(p!=NULL)
    {
        cout << p->data << " ";
...
//=====================COUNT======================================
int count(struct Node *p)                               // struct Node *p -> is the pointer to the first node!
{
    int l = 0;
    while(p)
    {
...
int Rcount(struct Node *p)                              // struct Node *p -> is the pointer to the first node!
{
    if(p)
...
//=======================SUM======================================
int sum(struct Node *p)                                 // struct Node *p -> is the pointer to the first node!
{
    int sum = 0;
    while (p){
        sum += p->data;
...
//=========================MAX====================================
int max(struct Node *p)                                 // struct Node *p -> is the pointer to the first node!
{
    int max = -32768;
    while(p)
    {
...
//=================NORMAL SEARCH====================================
struct Node * LSearch(struct Node *p, int key)          // struct Node *p -> is the pointer to the first node!
{
    while(p)
    {
        if(key==p->data)
...
struct Node * RecLSearch(struct Node *p, int key)       // struct Node *p -> is the pointer to the first node!
{
    if(p==NULL)
        return NULL;
...
struct Node * ModLSearch(struct Node *p, int key)           // struct Node *p -> is the pointer to the first node!
{
    struct Node *q = NULL;
    while(p)
    {
...
//========================INSERTING===============================
void Insert(struct Node *p, int index, int x)             // struct Node *p -> is the pointer to the first node!
{
    struct Node *t;

    if(index < 0 || index > count(p))   //check
        return;
    t = (struct Node *)malloc(sizeof(struct Node));
    t->data = x;
