...
struct Node *Delete(struct Node *t , int key)
{
    ...
    else
    {
        ...
        else if(bfactor(t) == 2 && bfactor(t->lchild) == -1)        // bfactor of t is 2 this means that left subtree has more no. of nodes than the right tree therefore we should check for bfactor of t->lchild
        {
            return LRrotation(t);
        }
